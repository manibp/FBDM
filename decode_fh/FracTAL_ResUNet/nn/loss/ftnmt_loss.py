...
class ftnmt_loss(Loss):
    ...
    def hybrid_forward(self,F, preds, labels, partial_mask=None):
       
        if partial_mask is not None:
          preds =F.broadcast_mul(preds,partial_mask)
          labels =F.broadcast_mul(labels,partial_mask)
          l1 = self.tnmt_base(F,preds,labels)
          preds_alt =F.broadcast_mul(1.-preds,partial_mask)
          labels_alt =F.broadcast_mul(1.-labels,partial_mask)
          l2 = self.tnmt_base(F,preds_alt, labels_alt)
