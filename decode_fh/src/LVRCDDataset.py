...
class LVRCDDataset(dataset.Dataset):
    def __init__(self, root=r'/content/drive/My Drive/Colab Notebooks/farm_hand_data/', mode='train', mtsk = True, transform=None, norm=None, pMessUp=0.0, Filter=256, prob_swap=0.5, prob_zero_change=0.5):
        
        self.NClasses=2
        self.NChannels=4 # RGB 
        ...
        if mode is 'train':
            flname_idx = glob.glob(root + r'training_FBDM_F{}.idx'.format(Filter))[0]
            flname_rec = glob.glob(root + r'training_FBDM_F{}.rec'.format(Filter))[0]
        elif mode is 'val':
            flname_idx = glob.glob(root + r'validation_FBDM_F{}.idx'.format(Filter))[0]
            flname_rec = glob.glob(root + r'validation_FBDM_F{}.rec'.format(Filter))[0]
        else:
    ...
    def __getitem__(self, idx):
        ...
        base = imgall[:self.NChannels].astype(np.float32)
        mask = imgall[self.NChannels:].astype(np.float32)
