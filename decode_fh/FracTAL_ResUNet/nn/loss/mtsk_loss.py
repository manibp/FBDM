...
class mtsk_loss(object):
    ...
    def __init__(self,depth=0, NClasses=2, farmhand=False):
    ...
    def loss(self,_prediction,_label):
        ...
        label_dists = _label[:,2*self.skip:3*self.skip,:,:]

        if self.farmhand:
            
            partial_mask = _label[:,3*self.skip:,:,:]

            loss_segm  = self.ftnmt(pred_segm,  label_segm, partial_mask)
            loss_bound = self.ftnmt(pred_bound, label_bound, partial_mask)
            loss_dists = self.ftnmt(pred_dists, label_dists, partial_mask)
        else:
