...
def get_partial(labels, _kernel_size = (3,3)):
    ...
    for channel in range(label.shape[0]):
        # label[channel] = cv2.erode(label[channel], cv2.getStructuringElement(cv2.MORPH_CROSS,_kernel_size) ,iterations = 1)
...
class WriteDataRecordIO(object):
    def __init__(self,
                 ListOfFlnames123,
                 flname_prefix_write= r'/content/drive/My Drive/Colab Notebooks/Farmhand_Data/',
        ...
        flname_train_idx = flname_prefix_write + r'training_FBDM_F{}.idx'.format(Filter)
        flname_train_rec = flname_prefix_write + r'training_FBDM_F{}.rec'.format(Filter)

        flname_valid_idx = flname_prefix_write + r'validation_FBDM_F{}.idx'.format(Filter)
        flname_valid_rec = flname_prefix_write + r'validation_FBDM_F{}.rec'.format(Filter)
    ...
    def update_imgs_mask(self, flnames123):
        name_img1, name_mask = flnames123


        with rasterio.open(name_img1,mode='r') as src1:
            img = src1.read()
            
        # with rasterio.open(name_img2,mode='r',driver='png') as src2:
        #     img2 = src2.read()

        with rasterio.open(name_mask,mode='r') as srcm:
    ...
    def _2D21H(self,tmask_label):
        ...
        if self.farmhand:
          tmask_label_1h_cp = tmask_label_1h  #create copy of extent mask
          tmask_label_1h_cp[0,:,:] = tmask_label_1h[1,:,:] # Replicate the layer 1 of extent mask for layer 0 as well
          partial_map = get_partial(tmask_label_1h_cp) # Run through the get_partial function to create a binary partial mask that covers extent and boundary area of field
          tmask_label_1h[0,:,:] = bounds_map[1,:,:] #copy distance mask as non-field extent mask for farm hand data
          ...
          distance_map = get_distance(tmask_label_1h) #get distance mask
